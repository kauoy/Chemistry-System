	...
	<div class="detail-preview" :style='{"width":"1200px","padding":"0 0 24px","margin":"0px auto","position":"relative"}'>
		...
		<el-tabs class="detail" :style='{"border":"1px solid #dfdfdf","minHeight":"350px","boxShadow":"1px 2px 3px #eee","padding":"16px","borderRadius":"16px","background":"#FFF"}' v-model="activeName" type="border-card">
			<el-tab-pane label="化工品详情" name="first">
                ...
                <!-- 价格历史走势图 -->
                <div v-if="priceHistoryData.length > 0" class="detail-item" :style='{"padding":"20px 0px","margin":"20px 0 0 0px","borderTop":"1px solid #eee","background":"none"}'>
                  <div class="title" :style='{"padding":"10px 0","fontSize":"16px","fontWeight":"bold"}'>历史价格走势</div>
                  <div id="priceHistoryChart" style="width: 100%; height: 300px;"></div>
                </div>
			</el-tab-pane>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        // 获取价格历史数据
        getPriceHistory() {
          this.$http.get('shangpinpricehistory/history/' + this.detail.id, {}).then(res => {
            if (res.data.code == 0) {
              this.priceHistoryData = res.data.data;
              
              // 如果数据不为空并且echarts已加载，渲染图表
              if (this.priceHistoryData.length > 0 && window.echarts) {
                this.$nextTick(() => {
                  this.renderPriceChart();
                });
              }
        ...
        renderPriceChart() {
          ...
          this.priceChart = window.echarts.init(chartDom);
          
          // 格式化日期和价格数据
          const dates = [];
          const prices = [];
          
          this.priceHistoryData.forEach(item => {
